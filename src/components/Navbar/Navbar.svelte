...
<div class="bg-black">
	<header class="relative bg-black shadow-lg">
		<nav aria-label="Top" class="container mx-auto px-4 sm:px-6 lg:px-8">
			<div class="border-b border-black">
				<div class="h-16 flex items-center">
					<!-- Logo -->
					<div class="logo ml-4 flex lg:ml-0">
						<a href="/">
							<span class="sr-only">Space X</span>
					...
					<div class="hidden lg:ml-8 lg:block lg:self-stretch">
						<div class="h-full flex space-x-8">
							{#each navItems as item}
								<a
									href="/"
									class="flex capitalize items-center text-sm font-medium text-white hover:text-gray-800"
									on:click|preventDefault={() => (active = item.id)}>{item.name}</a
								>
							{/each}
						</div>
					</div>
